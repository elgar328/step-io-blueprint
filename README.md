...
| `prune --corpus <path>` | `prune_overrides.toml` (선택, ABSTRACT supertype keep) | `variants.toml`, `arenas_overrides.toml` | **외부 STEP corpus** (`<path>` 안의 `*.sqfs` 컨테이너들) | `usage.toml`, `variants_pruned.toml`, `arenas_pruned.toml` |
...
### 외부 corpus 의존 (`prune` 만)

`prune` 은 53k 규모의 STEP 파일 corpus 를 walk 해 entity 인스턴스
...
`shape` / 후속 stage 가 자동 반영. 다른 stage 는 외부 의존이 없다.
...
- **`arena`** — variants 분류를 group (같은 enum 으로 묶이는 entity
  ...
  3-bucket 처리 없음.
- **`prune --corpus <path>`** — corpus 의 instance 카운트로
