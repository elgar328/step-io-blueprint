...
use crate::infer::arena::{ArenaSpec, compute_entity_to_group};
...
fn write_entities_toml(entities: &BTreeMap<String, EntitySummary>) -> Result<(), String> {
    let mut outer: BTreeMap<&str, &BTreeMap<String, EntitySummary>> = BTreeMap::new();
    outer.insert("entity", entities);
    let body =
        toml::to_string_pretty(&outer).map_err(|e| format!("serialize entities.toml: {e}"))?;
